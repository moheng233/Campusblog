...
export class ClientApi {
  ...
  async api<I extends {}, R extends {}>(
    ...
    let F = await fetch(
      `${this.HTTPHeader}${url}?${
        new URLSearchParams(this.removeEmpty(searchparams))?.toString() ?? ""
      }`,
      RI
    ).then(async (r) => {
      if (r.ok) {
        ...
        if (r.status == 401 && b.code == "token_not_valid" && token === true) {
          let r = await this.AuthRefToken(get(Login.RefToken));
   ...
   * @param info 用户信息
   */
  async UserSetInfo(info: { last_name: string; email: string }) {
    return await this.api<
      {
        last_name: string;
        email: string;
      ...
      IUser
    >(`/auth/users/me/`, undefined, info, "PUT", true);
  }
   ...
   */
  async ClassifyList() {
    let r = await this.api<{}, IClassify[]>(
      `/classify/`,
      undefined,
      undefined,
      "GET",
  ...
  async BlogList(
    page?: number,
    order: "updated_at" | "-updated_at" | "-fabulous" = "-fabulous",
    classify?: number,
    search?: string
  ) {
    let r = await this.api<
    ...
    >(
      `/blogs/`,
      { page: `${page}`, classify: classify, ordering: order, search: search },
      undefined,
      "GET",
      false
    );
  ...
  async GetImagesListByUser() {
    return await this.api<undefined, IUploadImage[]>("/uploadimage/");
  }

  async GetNotices() {
    return await this.api<undefined, { id: number; content: string }[]>(
      "/notices/"
    );
  }

  async ReadedNotices(nid: number) {
    return await this.api<undefined, {}>(`/notices/${nid}/readed`);
  }
}
